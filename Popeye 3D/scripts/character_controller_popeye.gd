...
func _process(delta: float) -> void:
	...
	if Input.is_action_pressed("popeye_mover_derecha"):
		direction.x += 1
	if Input.is_action_pressed("popeye_mover_izquierda"):
		direction.x -= 1
	if Input.is_action_pressed("popeye_mover_atras"):
		if current_state == States.CLIMB:
			direction.y = 1
		else:
			direction.z -= 1
	if Input.is_action_pressed("popeye_mover_adelante"):
		if current_state == States.CLIMB and not $RayCast.is_colliding():
			direction.y = -1
		else:
			direction.z += 1
	
	if Input.is_action_just_pressed("popeye_saltar") and $RayCast.is_colliding():
...
func _on_Body_area_entered(area: Area) -> void:
	match area.name:
		"CuerpoCorazon":
